...
const piecesImages: Record<'standard' | 'cats', Record<`${Color}${PieceSymbol}`, string>> = {
    standard: { bb: bbStandard, bk: bkStandard, bn: bnStandard, bp: bpStandard, bq: bqStandard, br: brStandard, wb: wbStandard, wk: wkStandard, wn: wnStandard, wp: wpStandard, wq: wqStandard, wr: wrStandard },
    cats: { bb: bbCat, bk: bkCat, bn: bnCat, bp: bpCat, bq: bqCat, br: brCat, wb: wbCat, wk: wkCat, wn: wnCat, wp: wpCat, wq: wqCat, wr: wrCat },
}

export const PIECES_STYLE_KEY = 'selectedPiecesStyle'
export let piecesStyle = $state<{ selected: keyof typeof piecesImages }>({ selected: (localStorage.getItem(PIECES_STYLE_KEY) ?? 'standard') as keyof typeof piecesImages })
